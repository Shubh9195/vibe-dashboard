...
export default async function QuizzesPage(props: { searchParams: Promise<{ filter?: string }> }) {
  ...
  const displayQuizzes = quizzes?.length ? quizzes : [
    { id: "mock-1", title: "IIT Delhi Technical Superintendent Full Mock", exam_type: "IIT", difficulty: "Hard", time_limit_minutes: 60, is_published: true, question_ids: Array(100).fill('') },
    { id: "mock-2", title: "NIT Trichy Admin Assistant Stage 1", exam_type: "NIT", difficulty: "Medium", time_limit_minutes: 120, is_published: true, question_ids: Array(100).fill('') },
    { id: "mock-3", title: "NFSU Computer Lab Attendant Skill Test", exam_type: "NFSU", difficulty: "Medium", time_limit_minutes: 45, is_published: true, question_ids: Array(100).fill('') },
  ];
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 mb-2">Institute Mock Tests</h1>
          <p className="text-slate-500 font-medium">Select a recruitment mock test below to begin your preparation under timed conditions.</p>
        </div>
      </div>

      <div className="flex gap-2 mb-8 overflow-x-auto pb-2 scrollbar-hide">
        {["All", "IIT", "NIT", "NFSU", "RRU", "Central Univ"].map(tab => (
