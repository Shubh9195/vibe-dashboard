  ...
  useEffect(() => {
    async function getUser() {
      ...
      if (user) {
        ...
        setUser(profile || { full_name: user?.user_metadata?.full_name || 'Student', exam_choice: 'UPSC CSE', streak: 3 });
      } else {
        setUser({ full_name: 'Rahul', exam_choice: 'SSC CGL', streak: 5 }); // Fallback
      }
    }
    getUser();
  }, [supabase]);
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-3">
          ...
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">State Rank</p>
            <h3 className="text-2xl font-black text-slate-900">#402</h3>
          </div>
        </div>
      </div>
